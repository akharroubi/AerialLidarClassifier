...
- Model selector in the dock and a `MODEL` parameter in the Processing
  ...
  model (dock, Processing or `qgis_process`) fetches whatever is still
  missing, with progress and cancel. There is no download step for the
  user; "import a weights file" stays for offline machines. A model
  that cannot run on the current device says why and offers a one-click
  switch to one that can.
...
### Dock and setup
- Model strip on two rows so model names are no longer clipped: the
  model and the import icon, then a coloured status (ready, weights
  download on first run, needs an NVIDIA GPU, LitePT dependencies
  missing) with a one-click "Use SegFormer 3D" when the selected model
  cannot run. On a GPU without spconv support (RTX 50) the dock opens
  on SegFormer 3D.
- A hint under the progress bar says what is still missing before Run
  (files, output folder, a model that runs here), or what Run will do.
- Run cannot be started twice while a run is in progress.
- A finished run reports its elapsed time; the QGIS message bar offers
  "Open folder" (and the course link unless it was hidden).
- Setup states which models the machine will get, downloads the
  weights, and after a failed GPU installation offers "Install the CPU
  version instead" (never chosen silently). The "torch already loaded"
  guard only blocks when the plugin's own torch is loaded.
- The provenance VLR of a re-classified file replaces the previous one
  of the same field instead of stacking.
- The Processing provider failing to register no longer prevents the
  dock from loading.
- QGIS 4 first-run setup: QGIS 4's launcher sets `PYTHONEXECUTABLE`,
  which leaked into `python -m venv` and uv, so every package landed in
  the portable Python instead of the plugin's environment and QGIS could
  not import them. The installer's subprocesses no longer inherit it, and
  Setup refuses to report "ready" unless the packages are inside the
  environment QGIS loads from.
- The release ZIP keeps the published package folder
  `Aerial_LiDAR_Classifier` (`build_zip.py` at the repository root,
  deterministic, SHA-256 sidecar).

### Release hardening
- Atomic LAS/LAZ publication; reject batch source/output aliases and duplicate outputs.
- Preserve raw scaled extra bytes (including uint64) and EVLRs in streaming and custom-label paths.
- Reject truncated inputs, incomplete predictions and protected/incompatible label fields.
- Custom fields retain raw model IDs; outputs include model provenance.
- Resolve vertical GeoTIFF CRS units and refuse geographic coordinates even with linear overrides.
- Register Processing in headless QGIS; retain RGB/legacy scan angles on required format upgrades.
- Wheel-only dependency installation, permanent TLS verification, explicit CPU mode and no silent CUDA fallback.
- Validate Python runtimes before promotion; safely accept internal archive links on older Python.
- Verify cached, migrated and loaded weights; failed promotions retain previous weights.
...
- The installer applied the torch version cap number to torchvision as
  ...
  constraints file the moment it is installed; torchvision resolves under that pin.

### Changed
- Existing environments are rebuilt once on first use (install schema
  4), which also moves them from cu128 to cu126 where relevant.
