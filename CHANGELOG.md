...
- New icon: a classified profile (ground, tree, building, power line,
  pole) in the plugin's ASPRS colours, textured as a point cloud, under
  an aerial scan cone. It reads as three coloured masses at 16 px on
  light and dark themes. SVG master and PNG exports in `assets/`,
  generated by `generate_logo.py` (repository root, outside the plugin).
